...
class UserController extends ivController
{
	...
	function editAction()
	{
		...
		if ($user) {
			...
			ivMessenger::add('error', "用户 $login 未找到");
			$this->_redirect('index.php?c=user');
		} else {
			$crumbs->push('add', 'index.php?c=user&amp;a=edit');
		}
	...
	function deleteAction()
	{
		...
		if (ivAcl::isAdmin($login)) {
			...
			if ($adminCount < 2) {
				ivMessenger::add('error', "用户 $login 是仅有的管理员用户，你不能删除它");
				$this->_redirect('index.php?c=user');
			}
		}
		$result = $this->_userManager->deleteUser($login);
		if ($result) {
			ivMessenger::add('notice', "用户 $login 删除成");
		}
		$this->_redirect('index.php?c=user');
	}
