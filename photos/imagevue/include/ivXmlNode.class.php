...
class ivXmlNode
{
	...
	function toString($valuesOnly = false, $depth = 0)
	{
		...
		if (($this->getValue() === null) && !$this->hasChildren()) {
			...
			if ($this->hasChildren()) {
				...
				$result .= htmlspecialchars($this->stripInvalidChars($this->_getSerializedValue()), ENT_COMPAT, 'UTF-8');
			}
			$result .= '</' . $this->getName() . '>';
			$result .= "\r\n";
		}
	...
	/**
	 * Strips invalid XML characters
	 * via http://cse-mjmcl.cse.bris.ac.uk/blog/2007/02/14/1171465494443.html
	 *
	 * @param  string $string
	 * @return string
	 */
	...
	{
		return preg_replace('/[^\x9\xA\xD\x20-\x{D7FF}\x{E000}-\x{FFFD}\x{10000}-\x{10FFFF}]/u', '', $string);
	}	
